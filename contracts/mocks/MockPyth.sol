...
contract MockPyth is OwnableUpgradeable {
    mapping(bytes32 => IPyth.PriceFeed) private priceFeeds;
    ...
    function getPriceUnsafe(
        bytes32 id
    ) public view returns (IPyth.Price memory price) {
        IPyth.PriceFeed memory priceFeed = queryPriceFeed(id);
    ...
    function queryPriceFeed(
        bytes32 id
    ) public view returns (IPyth.PriceFeed memory priceFeed) {
        require(
            priceFeeds[id].id != 0,
    ...
    function _updatePriceFeeds(bytes[] memory updateData) internal {
        ...
        for (uint i = 0; i < updateData.length; i++) {
            IPyth.PriceFeed memory priceFeed = abi.decode(
                updateData[i],
                (IPyth.PriceFeed)
            );
    ...
    function createPriceFeedUpdateData(
        ...
        IPyth.PriceFeed memory priceFeed;
